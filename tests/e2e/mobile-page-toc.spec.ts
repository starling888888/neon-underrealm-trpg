...
test("mobile page toc opens and closes from its trigger @mobile-page-toc-toggle @local-fixture", async ({
  page,
}) => {
  await page.setViewportSize(siteViewports.mobile);
  await page.goto(siteRoutes.mdxTest);
...
test("mobile page toc heading stays sticky @mobile-page-toc-scroll-behavior @local-fixture", async ({
  page,
}) => {
  await page.setViewportSize(siteViewports.mobile);
  await page.goto(siteRoutes.mdxTest);
...
for (const [name, viewport] of [
  ["tablet", siteViewports.tablet],
  ["mobile", siteViewports.mobile],
] as const) {
