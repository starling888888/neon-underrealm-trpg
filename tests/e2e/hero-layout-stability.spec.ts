...
const staticHeroRoutes = [
  siteRoutes.world,
  siteRoutes.advancement,
  siteRoutes.characterMaking,
  siteRoutes.rules,
  siteRoutes.scenarioPlay,
  siteRoutes.battle,
  siteRoutes.data,
  siteRoutes.dataRyugi,
  siteRoutes.dataIkizama,
  siteRoutes.dataItems,
  siteRoutes.dataItemsWeapons,
  siteRoutes.dataItemsArmors,
  siteRoutes.dataItemsOmamori,
  siteRoutes.dataItemsCybernetics,
  siteRoutes.dataItemsNanomachines,
  siteRoutes.dataItemsDrugs,
] as const;

const dynamicHeroRoutes = [
  siteRoutes.dataRyugiDetail("kenkaya"),
  siteRoutes.dataIkizamaDetail("burai"),
] as const;

const imageBasePath = new URL(siteBaseUrl).pathname.replace(/\/$/u, "");
...
const layoutShiftScenarios = [
  ...
  {
    name: "dynamic hero",
    route: siteRoutes.dataRyugiDetail("kenkaya"),
...
for (const scenario of layoutShiftScenarios) {
  for (const [viewport, viewportName] of [
    [siteViewports.desktop, "desktop"],
    [siteViewports.mobile, "mobile"],
  ] as const) {
