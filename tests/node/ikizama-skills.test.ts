...
describe("ikizama skill conversion", () => {
  it("aggregates owner sheets and regenerates after owner additions or removals", async () => {
    ...
    await workbook(fixture.input, [
      ...
      { name: "kejime", rows: [headers, row("bonus", "義肢", "Pv")] },
    ]);
    const added = await convertIkizamaSkills({
      inputPath: fixture.input,
      outputPath: fixture.output,
  ...
  it("rejects missing and unexpected owner sheets", async () => {
    await using fixture = await createFixture();
    await workbook(fixture.input, [
      { name: "burai", rows: [headers, row("basic", "一撃", "Pv")] },
...
async function createFixture() {
  const directory = await mkdtemp(join(tmpdir(), "ikizama-skills-"));
  return {
    input: join(directory, "ikizama-skills.xlsx"),
    output: join(directory, "ikizama-skills.json"),
