...
### 2. UI実装前にdesign画像を生成する

UI、CSS、layout、page、Componentを実装するタスクでは、実装開始前に対象画面またはComponentのdesign画像を生成する。

生成したdesign画像は、Visual Reviewの正本として `docs/design/<design-target>/` 配下に配置する。

UI実装タスクのissueファイルには、参照するdesign targetと主要なdesign画像を記載する。

design画像が未生成の場合、UI実装には進まず、先にdesign生成を行うか、ユーザーに確認する。
...
### 7. `docs/plan.md` のチェックは勝手に完了扱いしない
