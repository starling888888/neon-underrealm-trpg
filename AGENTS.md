...
- 無料枠を不要に消費しないため、developmentを含む実環境のAPI（Cloudflare Worker URL、remote D1/R2、その他の外部service）へのrequestまたはremote操作は、ユーザーの明示許可なしに実行してはならない。local binding、Miniflare、workerdを使うtestはこの制約の対象外とする。
