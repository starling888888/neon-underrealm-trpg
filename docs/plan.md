...
- [ ] `20-2-introduction-page` — はじめにページを作成する
  ...
  - [ ] `.raw/contents/introduction.md` のfrontmatter、Markdown本文、HTMLコメント指示をもとに画面を作成する
  - [ ] ゲーム概要を再掲せず、ページの役割を示す短い導入文、必要なもの、基本用語、ゴールデンルール、読み始める導線を配置する
  - [ ] ゴールデンルールを注意枠のCalloutで表示する
...
- [ ] `21-2-world-page` — ワールドガイドページを作成する
  ...
  - [ ] designを生成する
  - [ ] `/world.mdx` を作成する
  - [ ] `.raw/contents/world.md` のfrontmatter、Markdown本文、HTMLコメント指示をもとに画面を作成する
  - [ ] 初期公開範囲の世界観本文を配置する
...
- [ ] `22-2-character-making-page` — キャラクターメイキングページを作成する
  ...
  - [ ] `.raw/contents/character-making.md` のfrontmatter、Markdown本文、HTMLコメント指示をもとに画面を作成する
...
- [ ] `23-2-rules-page` — ルールトップページを作成する
  ...
  - [ ] `.raw/contents/rules.md` のfrontmatter、Markdown本文、HTMLコメント指示をもとに画面を作成する
  - [ ] `/introduction` のゴールデンルールへの参照を配置する
  - [ ] 判定、達成値、効果値、対抗判定、端数処理を配置する
  - [ ] 必要に応じてCalloutを配置する
  - [ ] 完成画面のスクリーンショットを取得し、design正本を更新する
...
- [ ] `24-2-scenario-play-page` — シナリオ進行ルールページを作成する
  ...
  - [ ] `.raw/contents/scenario-play.md` のfrontmatter、Markdown本文、HTMLコメント指示をもとに画面を作成する
...
- [ ] `25-2-battle-page` — 戦闘ルールページを作成する
  ...
  - [ ] `.raw/contents/battle.md` のfrontmatter、Markdown本文、HTMLコメント指示をもとに画面を作成する
...
- [ ] `26-2-advancement-page` — 成長ページを作成する
  ...
  - [ ] `.raw/contents/advancement.md` のfrontmatter、Markdown本文、HTMLコメント指示をもとに画面を作成する
  - [ ] キャラクター成長に関する本文を配置する
  - [ ] 必要に応じてCalloutを配置する
  - [ ] 完成画面のスクリーンショットを取得し、design正本を更新する
...
- [ ] `27-2-data-index-page` — データトップページを作成する
  ...
  - [ ] `.raw/contents/data.md` のfrontmatter、Markdown本文、HTMLコメント指示をもとに画面を作成する
...
- [ ] `28-2-common-skills-page` — 共通スキル一覧ページを作成する
  ...
  - [ ] `.raw/contents/common-skills.md` のfrontmatter、Markdown本文、HTMLコメント指示をもとに画面を作成する
  - [ ] 共通スキル一覧データを表示する
...
- [ ] `42-install-pagefind` — Pagefindを導入する

  - [ ] Pagefind package追加
  - [ ] build後にindex生成できる
  - [ ] `npm run index:search` 追加

- [ ] `43-search-modal-ui` — 検索モーダルUIを作成する
...
- [ ] `44-search-pagefind-integration` — Pagefind検索連携を実装する

  - [ ] 検索語入力でPagefind検索
  - [ ] 検索結果をモーダル内に表示
  - [ ] 結果クリックで該当ページまたはアンカーへ遷移

- [ ] `45-search-metadata` — 検索対象・除外・メタデータを調整する

  - [ ] ヘッダー、フッター、サイトメニュー、ページ内目次を検索対象から除外
  - [ ] ページタイトル、セクション、種別ラベルを検索結果に表示
  - [ ] データカード個別アンカーが検索結果から利用できることを確認する

- [ ] `46-search-mobile-behavior` — スマホ検索挙動を調整する
...
- [ ] `47-search-index-ci-deploy` — CIを更新して検索index生成込みでデプロイする
...
- [ ] `48-accessibility-pass` — 最低限アクセシビリティを確認する
...
- [ ] `49-responsive-pass` — レスポンシブ調整を行う
...
- [ ] `49-1-vrt-css-regression-guards` — VRT導入時にレイアウトCSS回帰検知を追加する
...
- [ ] `51-github-pages-base-check` — GitHub Pagesサブパス確認を行う
...
- [ ] `52-content-smoke-test` — 主要ページ表示確認を行う
...
- [ ] `53-release-docs` — 公開手順ドキュメントを整備する
...
- [ ] `54-initial-release` — 初期公開用最終調整を行う
...
- [ ] `55-ci-non-main-branches` — main以外でdeployなしCIを回すためのテスト / CIを整備する
