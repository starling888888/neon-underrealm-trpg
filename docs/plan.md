...
- [x] `27-2-data-index-page` — データトップページを作成する
  ...
  - [ ] アイテム凡例は`34-2-items-pages`で、各Item系Cardに凡例用データを渡して表示する
  - [x] 完成画面のスクリーンショットを取得し、design正本を更新する

- [ ] `34-2-items-pages` — 6種別のアイテムページをまとめて作成する

  - [ ] 6ページを同一issue・branch・PRで実装し、共通するページ構造とデータ取得をまとめて扱う
  - [ ] `WeaponCard`、`ArmorCard`、`OmamoriCard`、`CyberneticCard`、`NanomachineCard`、`DrugCard`の既存表示契約と整合させる
  - [ ] 次の6 routeを作成し、各種別の一覧と対応するCardの凡例を表示する
    - [ ] `/data/items/weapons.astro` — 武器 / `WeaponCard`
    - [ ] `/data/items/armors.astro` — 防具 / `ArmorCard`
    - [ ] `/data/items/omamori.astro` — お守り / `OmamoriCard`
    - [ ] `/data/items/cybernetics.astro` — サイバネ / `CyberneticCard`
    - [ ] `/data/items/nanomachines.astro` — ナノマシン / `NanomachineCard`
    - [ ] `/data/items/drugs.astro` — ドラッグ / `DrugCard`
  - [ ] 各ページで対応するcontents inputのfrontmatter、Markdown本文、HTMLコメント指示を表示構成の正本として使う
    - [ ] `.raw/contents/items-weapons.md`
    - [ ] `.raw/contents/items-armors.md`
    - [ ] `.raw/contents/items-omamori.md`
    - [ ] `.raw/contents/items-cybernetics.md`
    - [ ] `.raw/contents/items-nanomachines.md`
    - [ ] `.raw/contents/items-drugs.md`
  - [ ] 各ページのVisual Test用スクリーンショットを取得し、種別ごとのdesign targetをcanonicalizeする
