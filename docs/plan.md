...
- [ ] `21-2-world-page` — ワールドガイドページを作成する
  ...
  - [ ] 初期公開範囲の世界観本文、強大な敵、NPC紹介を配置する
  - [ ] 共通のシンプルな人物アイコン、二つ名、名前、説明を表示する静的 `NpcCard` をこのタスクで作成する。二つ名がある場合はルビ付きの控えめな青緑寄りの文字色で名前の直前に続け、ない場合は名前から表示する
  - [ ] NPCをExcel / JSON管理へ移行せず、MDX内の静的propsで `NpcCard` を配置する
...
- [ ] `48-search-index-ci-deploy` — CIを更新して検索index生成込みでデプロイする
