...
- `docs/issue/milestone-<NN>/plans/`: 完了した親issueの軽量なGate plan。各完了Gateは子issue名とGitHub Issue番号だけを参照する。
...
レビュー出力、比較メモ、外部snapshotは `.tmp/` に置く一時入力である。確認済みの要件、判断、後続対応だけをissue、TODO、requirements、milestone planへ反映する。完了済みissueの最終記録は同名のGitHub closed Issueへ残し、ローカルのissue fileは削除する。milestone planと完了したGate planには、`<issue-slug> — GitHub Issue #<number>` だけを履歴として残す。
