...
| 変換コマンド     | `npm run convert:ryugi-skills`        |
| データ取得層     | `src/lib/data/ryugi-detail.ts`        |
...
同一データを再変換した場合は、既存JSONの`updatedAt`を維持する。流儀ID集合の追加・削除などで
...
- 各スキルIDが所属流儀ID、カテゴリ、正規化タイミング、名称hashと一致し、全体で一意であること
...
- 流儀ごとの名称hash ID、行の途中挿入と並び替えでIDが変わらないこと、流儀ごとの`sourceOrder`、カテゴリ配列の表示順を検証すること
