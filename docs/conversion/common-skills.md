...
同じグループ内の並びは自由とする。攻撃グループでは、攻撃タイミングの詳細同士にWarning順を設けない。途中のグループを省略してよい。前に出現したグループより優先順位が高いグループが後から現れた場合は、その行について警告する。たとえば、`Pv` の後に `R` が現れた場合や、`R` の後に攻撃タイミングが現れた場合が対象である。
...
- トップレベルの厳密なフィールド構成
- `dataName`、ISO 8601かつJSTオフセット `+09:00` を持つ `updatedAt`
