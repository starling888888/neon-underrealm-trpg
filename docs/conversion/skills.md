...
入力の`使用制限`は1行の文字列として受け入れ、同じ表記のまま生成JSONへ出力する。使用回数を表す
`回数/区分`の構文は、`&`で連結できる。`回数`は正の整数、`lv`、または`LV`、`区分`は次の日本語略号
だけを受け入れる。
...
`特殊`や`タイミングMのスキル`のように`/`を含まない個別の使用制限は、そのまま受け入れる。`/`を含む値は
上記の構文に一致しなければ変換エラーとする。変換時に使用制限の表記は置換しない。
