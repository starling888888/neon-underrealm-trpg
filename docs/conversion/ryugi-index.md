...
- 流儀名称は全データで一意でなければならない。
- IDは後続の`/data/ryugi/[ryugiId]`と、流儀スキルの所属流儀IDを結び付ける基準とする。
- 流儀の名称とA列で決定したIDの組は恒久的に不変とする。既存流儀の名称変更、ID変更、削除は認めない。
- 追加は許可する。ただし新規流儀は、既存流儀と重複しない名称とIDを持たなければならない。
- 既存の`data/generated/ryugi-list.json`がある再変換では、既存の全IDが出力にも残り、各IDに対応する名称が変わらないことを検証する。IDが消えた場合、既存IDに別の名称を対応させた場合、既存名称に別のIDを対応させた場合は変換エラーとする。
...
- 既存流儀の名称変更、ID変更、削除を拒否し、新規流儀の追加を許可すること
