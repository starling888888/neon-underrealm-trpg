...
### Repeated check failure while adding item conversion

#### 2026-07-22

- source: agent self-report
- 発生箇所: `34-0-items-data` のItem Schema・変換・テスト追加後の`npm run check`
- 観測した失敗: 初回の型チェックで`WeaponKind`型のexport漏れが発生した。修正後の再実行では、新規converterのimport順、`??=`を含む代入式、テストfixtureの未使用変数がBiome規約に違反して停止した。同じ実装作業中に`check`失敗を2回発生させた。
- 一次対応: Schemaを参照するconverterを追加した直後に、exportした型を確認する。新規の配列・Record初期化は代入式内で行わず、Biomeのimport整理を実行してから全体checkを再実行する。
