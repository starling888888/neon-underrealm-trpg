...
- 3レールPCレイアウト: `1280px` 以上
- タブレット / 狭幅PC: `768px` 以上 `1280px` 未満
- スマホレイアウト: `768px` 未満

`1280px` 以上では、左にサイトメニュー、中央に本文、右にページ内目次を表示する。左右のレールはそれぞれ `15rem`、本文は最小 `46rem` を目安とする。

`768px` 以上 `1280px` 未満では、左のサイトメニューは常設表示する。右サイドのページ内目次は常設表示せず、本文見出し横の開閉式UIに切り替える。

`768px` 未満では、サイトメニューも常設表示せず、ヘッダーから開閉するUIに切り替える。

最終的なブレークポイントはUI確認後に調整してよい。右サイド目次、開閉式ページ内目次、sticky見出し、scroll paddingの切替は同じ基準幅にそろえる。
...
- ヒーロー画像は、画像の表示完了前から表示領域を確保し、本文コンテンツの位置がずれないようにする
...
初期実装では、明示的な再利用許諾をしない場合でも、必要な著作権者表記を表示できる構成にする。Footerからのクレジット導線は初期必須にしない。
