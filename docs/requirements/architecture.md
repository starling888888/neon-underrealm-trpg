...
初期公開スコープではPL向け本文を主対象とし、シナリオ本文、キャンペーン本文は含めない。
...
NPC一覧・詳細、シナリオ本文、キャンペーン本文は初期実装対象に含めない。
...
- character sheet cloud persistence backendのAPI、dependency境界、mock repositoryを使うservice test、error contract、local/productionのCloudflare repository生成の詳細は`docs/architectures/backend.md`を正本とする。
