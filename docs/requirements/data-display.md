...
カードは縦長で、Component designで定める最低高さを持つ。本文量に応じて高さを可変とし、長文でも過度に縦長にならないよう、概要と効果の本文は通常のページ本文より十分に小さい文字で表示する。

カード一覧は、スマホ幅では2列、デスクトップ幅ではカードの最低幅を保った3列または4列のgridで表示する。
...
```text
/data/ryugi/teppoudama#skill-ryugi-teppoudama-basic-a-001
