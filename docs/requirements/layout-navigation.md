...
`768px` 以上では左サイドバーとして常設表示する。

スマホ版では、ヘッダーのボタンから開閉できるドロワーまたはオーバーレイとして表示する。

ルート階層では、`ルール`を`データ`より上に表示する。この順序はPC版とスマホ版で共通とする。
...
目次対象の見出しが1件以上あるページでは、件数にかかわらずその見出しを目次へ表示する。対象見出しが0件のページでは、目次の見出し下に「見出しがありません」と表示する。
...
`1280px` 以上では、ページ内目次を右サイドに表示する。

ページ内目次は本文とは独立した固定位置に表示し、本文スクロール中も画面内に残ること。

`1280px` 未満では「このページの目次」をワンタッチで開き、目次項目から該当箇所へジャンプできること。
...
### FR-01-07. ページ下部の前後ナビゲーション

読書順を定義するページでは、本文末尾に前ページ・次ページへ移動するナビゲーションリンクを表示できること。

表示対象ページと前後関係は、ページ構成を踏まえて定義する。前後関係を持たないページでは、対応するリンクを表示しない。

スマホでも十分なタップ領域と視認性を確保し、サイトメニューやページ内目次とは異なるページ間移動の導線として扱う。
