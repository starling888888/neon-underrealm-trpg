...
- 縁は、対象と関係の列ヘッダーを置き、短い対象入力と長い関係入力を各行に並べる。覚悟の効果は縁の下に置き、`覚悟の効果`見出しの横に灰色の`通常の縁／今生の縁`を表示する。通常行のクリアは文字列でなく、透過背景の円形`×` icon buttonで示す。上限外の未覚悟行は、その他流儀と同じsolid circle `×` delete buttonとerror colorで示す。
