...
- 縁は、対象と関係の列ヘッダーを置き、短い対象入力と長い関係入力を各行に並べる。覚悟の効果は縁の下に置き、`覚悟の効果`見出しの横に灰色の`通常の縁／今生の縁`を表示する。クリアは文字列でなく、透過背景の円形`×` icon buttonで示す。
...
- 副能力値は1項目ずつ縦に並べるが、各項目内の`自動算出値 + 修正入力 = 最終値`は横並びを維持する。縁は行間と余白を縮める。縁の覚悟効果は、気絶からの回復、気合獲得、能動判定、受動判定の順を維持した2行2列に並べる。
...
- `覚悟の効果`見出しの横に灰色の`通常の縁／今生の縁`を置き、個別labelなしに`通常の縁使用時の元値 / 今生の縁使用時の元値 + 修正値 = 通常の縁使用時の最終値 / 今生の縁使用時の最終値`で示す覚悟効果
- 対象、関係、覚悟checkboxを持つ、縁最大数に応じた固定行。対象と関係は列ヘッダーで示し、行ごとのラベルは置かない。対象入力は短く、関係入力は長く取る。ポジティブ／ネガティブの種別は管理しない。覚悟checkboxのlabelには既存`FormulaTooltip`を付け、既存の`?`indicatorを含むtooltip triggerはcheckboxの操作targetとは分ける。tooltipは「シナリオ中、覚悟にした縁にチェックを入れます。チェックが入っている限り、変更もクリアもできません」と示す。追加・削除ではなく、行削除と誤認しない透過背景の円形`×` icon buttonでクリア操作を示す。覚悟済みの行は編集ロックと、チェック解除後に再編集できることを見た目で伝える。最大数が減っても既存入力は保持し、対象または関係が空でない、または覚悟チェック済みの行が上限を超える時は警告する。
