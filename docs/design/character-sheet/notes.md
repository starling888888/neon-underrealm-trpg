...
- desktopでは、右上の操作領域から現在のエラーをすべて確認できるようにする。
- tabletとmobileでは、右下のsticky controlから現在のエラーをすべて確認できるようにする。
- 色による通知に加え、入力のエラー状態を支援技術へ伝え、操作領域のエラー一覧にはテキストを置く。これにより、個々の入力欄へ可視のエラー文言を増やさずに、色だけへ依存しない。
...
- 通常の縁と今生の縁それぞれの覚悟効果値
- 対象、ポジティブまたはネガティブの種別、関係、覚悟checkboxを持つ追加可能な縁行。覚悟済みの行は編集ロックと、チェック解除後に再編集できることを見た目で伝える。
- 攻撃、リアクション、非戦闘の判定行。各行に5能力値すべての選択、手動修正、算出値、該当する非戦闘の得意技能操作を持つ。

### スキル

- プライマリ流儀、生き様、共通スキル、その他流儀を識別できる個別のスキル領域
- 所属マスタに限定した候補、ボーナススキルの自動表示、通常スキルの可変行、レベル入力、追加・削除・変更時の扱い
- 表示するが自動検証しないスキル説明と文章条件のための情報表示
