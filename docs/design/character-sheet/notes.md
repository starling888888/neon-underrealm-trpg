...
- desktopは`.tmp/character-sheet-design-draft.jpg`の左右二分配置を骨格とする。左カラムは、基本情報（経験点・信用を含む）、流儀・生き様と能力値、副能力値、縁、判定、武器・防具の順とする。右カラムは、上部操作、スキル、専用アイテムの順とする。
...
- desktopの副能力値は、体力系と精神力系、移動力系と行動値系、行動回数と縁最大数（それぞれの補正入力を含む）の3行に圧縮する。行動回数と縁最大数の補正は独立枠にせず、それぞれの表示枠内に置く。算出式と一時修正の適用checkboxは対象ラベルの横に置き、行を増やさない。各行の余白、入力高を抑え、流儀・生き様と能力値より目立たない密度にする。
- desktopの縁は、対象と関係の列ヘッダーを置き、短い対象入力と長い関係入力を各行に並べる。覚悟の効果は縁の下に移し、4効果を横並びにして各修正値の入力を短くする。
...
- Gate 0は、既存の表示文字列を解析せずに共通スキルボーナスの構造化データを追加する。UIは共通スキルボーナスや自由文のスキル効果が自動算出されるかのように表示してはならない。
...
- プライマリ流儀、生き様、共通スキル、その他流儀を識別できる個別のスキル領域。desktopでは横並びにせず、この順で右カラムへ縦に積む。
...
- desktopの専用アイテムは横に並べず、右カラムで縦に積む。各カテゴリは右カラム幅を使い、カテゴリ追加後の内容は下へ続けて表示する。
