...
- 2026-07-31、G31の方針として`frontend/canonical-snapshots/visual/**`をGit管理から全削除した。以後のcanonical baselineは同じpathへlocal-onlyで再生成し、Gitへ追加しない。
...
- 縁は、対象と関係の列ヘッダーを置き、短い対象入力と長い関係入力を各行に並べる。各行の左端には既存のスキル・武器防具と同じコンパクトなグレー背景の上下順序入れ替えcontrolを置く。移動できない方向のcontrolは表示しない。覚悟の効果は縁の下に置き、`覚悟の効果`見出しの横に灰色で`通常の縁／今生の縁`を表示する。通常行のclearは囲みのない消しゴムicon、上限外の未覚悟行とその他流儀の削除は囲みのないゴミ箱iconとerror colorで示す。上限外のtext inputは通常backgroundとdanger borderで示す。操作列をicon寸法に縮めた分は関係または名称列へ配分する。
...
- tabletとmobileでは、右下のsticky controlから開くメニュー内に、現在のエラー全件を直接表示する。エラー時はメニューbuttonと`エラーがN件あります。`をdangerカラーにし、error本文は通常本文色の順序なしリストで示す。閉じたmenu buttonのaccessible nameにも、開閉状態と`エラーはありません。`または`エラーがN件あります。`を含める。
- 色による通知に加え、入力のエラー状態を支援技術へ伝え、desktop dialogおよびtablet / mobile menuのエラー一覧にはテキストを置く。行ごとのerrorは、解決済みの名称と現在のLvまたは不正条件を一覧だけで示す。これにより、個々の入力欄へ可視のエラー文言を増やさずに、色だけへ依存しない。
...
- 対象、関係、覚悟checkboxを持つ、縁最大数に応じた固定行。対象と関係は列ヘッダーで示し、行ごとのラベルは置かない。各行の左端には既存スキル・武器防具と同じコンパクトな上下の順序入れ替えcontrolを置き、移動できない方向のcontrolは表示しない。対象入力は短く、関係入力は長く取る。ポジティブ／ネガティブの種別は管理しない。覚悟checkboxのlabelには既存`FormulaTooltip`を付け、既存の`?`indicatorを含むtooltip triggerはcheckboxの操作targetとは分ける。tooltipは「シナリオ中、覚悟にした縁にチェックを入れます。チェックが入っている限り、変更もクリアもできません」と示す。通常行は行削除と誤認しない角丸の横長`クリア` text buttonでクリア操作を示す。覚悟済みの行は編集ロックと、チェック解除後に再編集できることを見た目で伝える。最大数が減った時は空行を減らして入力済み行を保持し、入力済み行が上限を超える時は上限外行を通常backgroundのままerror borderとsolid circle `×` delete buttonで示す。覚悟済み行は削除できない。
...
- スキル候補ダイアログは、名称、最大Lv、タイミング、コスト、使用制限の順に共通headerと候補行を置く。タイミング列は最大Lvとコストの間に固定し、desktop、tablet、mobileのいずれでも値を折り返さない幅を確保する。
