...
- desktopは`.tmp/character-sheet-design-draft.jpg`の左右二分配置を骨格とする。左カラムは、基本情報（経験点・信用を含む）、副能力値、縁、流儀・生き様と能力値、判定、武器・防具の順とする。右カラムは、上部操作、スキル、専用アイテムの順とする。
...
- キャラクター設定は機能として維持するが、今回のdesktop draftでは入力欄を表示しない。最終配置は、折りたたみ式を候補に後続のdesign対話で決める。
...
- 改行を保持する複数行のプレーンテキストによるキャラクター設定。今回のdesktop draftでは配置を保留して入力欄を表示しない。
...
- 通常の縁と今生の縁それぞれの覚悟効果値
- 対象、関係、覚悟checkboxを持つ、縁最大数に応じた固定行。ポジティブ／ネガティブの種別は管理しない。追加・削除ではなくクリア操作を示す。覚悟済みの行は編集ロックと、チェック解除後に再編集できることを見た目で伝える。最大数が減っても既存入力は保持し、超過時は警告する。
...
- 各スキル行へ、名称選択、Lv、最大レベル、コスト、技能、使用制限を表示する。その直下に、取得制限、対象、射程、効果の折りたたみ領域を置く。効果は初期状態で折りたたみ、文章条件は表示するが自動検証しない。選択済みスキルのための別詳細枠は設けない。
