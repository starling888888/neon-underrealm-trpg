...
- デザイン確認用ビューポート:
  - tablet: `820x1180`
  - mobile: `390x900`
- 広幅desktop専用のレイアウトとVRTは初期scope外である。
...
- `.tmp/character-sheet-design-draft.jpg`は広幅desktopの過去の配置参考である。要件の正本ではなく、初期scopeのtabletまたはmobileのレイアウトを定めず、必須入力も網羅していない。

## 必須の画面範囲

後続のドラフトでは、以下の全グループを直接編集または読み取り専用の確認対象として利用可能にする。視覚的なグループ分けは変更できるが、過去の広幅desktop配置参考に存在しないことを理由に省略してはならない。
...
- ナノマシン、お守り、サイバネ、ドラッグを含む専用アイテムは、効果本文を初期表示しない。各行の`効果を展開`操作で効果を確認する。この方針はtabletとmobile、および後続の広幅desktop対応で共通とする。
...
- tabletとmobileは別々のレイアウト課題である。tabletの配置をそのまま縮小してmobileにしてはならない。
...
このノートは、これから行うデザイン対話の出発点である。後続のtablet、mobileに関するユーザー要望が実際のドラフトを定める。ドラフト承認後に限り、このノートを最終layout intentとVRT scenarioで更新し、その後に`ex-02`が求めるアーキテクチャと実装Gateの計画へ進む。
