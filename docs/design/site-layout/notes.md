...
- test: `tests/vrt/site-layout.spec.ts` の `@vrt @site-layout @<state> @<viewport>`
...
- visual direction: 白寄り本文面、暗めHeader / Footer、青緑accentを維持し、共通レイアウト / ナビゲーションを横断的な完成状態として扱う。
...
- `base-layout` の共通layout構造に、Header、Footer、navigationを組み合わせる。
...
- `/release-notes` は `docs/design/release-notes/` を正本として扱い、このdesign targetでは横断layout状態の代表routeには含めない。
- `/data/items/weapons/` は、MobileMenu open stateを比較する横断layout用fixtureである。
...
- `/404` のPageToc非表示状態をsite-layoutのVRTへ追加するか。
