...
- typography direction: H1とカテゴリH2で本文構造を示す。各カードは名称を最上段へ単独で置き、`最大LV`は名称行の下の独立行に置く。効果本文だけを通常本文より小さく表示する。長文を切り詰めない。
...
- `SkillCard` の名称、名称行下の最大LV、メタ情報、詳細枠、効果本文の表示順と可変高さを維持する。
