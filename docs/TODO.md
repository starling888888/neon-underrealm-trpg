...
JSONインポートbuttonを削除し、DB保存への移行を完了する。ex-17から移管したlocal draft / URL identity確認もこのissueで扱う。

- [ ] JSONインポートbuttonを削除する
  - source: ex-17 TODO「JSON import導線削除後にidなしlocal draft遷移を担う不要処理が残らないことを確認する」
  - JSON import導線の削除後に、不要となるidなしlocal draft遷移、画像処理、state・callback・URL identity関連処理が残っていないことを確認する。
  - local draft / URL identity contractを実装・test・design noteで整合させる。
