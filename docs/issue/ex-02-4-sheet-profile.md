...
Webキャラクターシートの`profile` slotに、PC名・二つ名・年齢・性別・PL名、開閉式のキャラクター設定、信用の入力と派生値表示を追加する。文字列の空欄と信用入力の整数制約を両立し、後続Gateが同じRHF formを拡張できる最小の編集値境界を定める。
...
入力値の型は`null`を許容せず、文字列入力のdefault valueは空文字列（`""`）とする。PC名・二つ名・年齢・性別・PL名とキャラクター設定は空欄を許可する。取得信用、融通した信用、融通された信用は`0`以上の整数だけを受け付け、小銭最大値修正は負数を許可する整数とする。信用の4入力は`0`をdefault valueとし、空欄になる操作では`0`へ戻す。
...
- `profile` slotへ、PC名、二つ名、年齢、性別、PL名の1行自由入力を表示する。
- 基本情報内のプロフィール入力群の直下に、初期状態で閉じた`設定`操作と、改行を保持するプレーンテキスト入力を追加する。設定の開閉状態はRHF値・保存・JSONの対象に含めない。
- 取得信用、融通した信用、融通された信用の`0`以上の整数入力、小銭最大値修正の負数を許可する整数入力、使用可能信用、消費信用、小銭最大値の読み取り専用表示を追加する。信用の4入力は`0`をdefault valueとし、空欄になる操作では`0`へ戻す。G4時点の消費信用は後続のアイテム入力がないため`0`として表示し、使用可能信用と小銭最大値は要件の式で算出する。
- G4で追加するform value、default value、Component props、算出入力へ`null`を持ち込まない。文字列は`""`をdefaultにして空欄を許可し、信用の4入力は`number`のまま扱う。
- `CharacterSheetContainer`のRHF default valuesと型、profile用Presenter / CSS Module、既存form shellのslot接続を必要範囲で追加する。form値はRHFを唯一の編集stateとし、別storeへ複製しない。
- 取得信用・融通した信用・融通された信用の`0`以上制約、小銭最大値修正の負数、信用欄の空欄操作が`0`へ戻ること、信用の派生式、設定の改行と開閉をNode testおよびユーザー観測可能なbrowser behaviorで確認する。
- desktop、tablet、mobileで基本情報が横overflowせず、labelと入力の対応、開閉操作、visible focusを保つ。UI変更後のPRレビュー直前に、G4で確定したVRT targetだけを比較する。
...
- [ ] G4向けのdesign intentとVRT参照情報を`docs/design/character-sheet/notes.md`へ記録し、ユーザー承認を受けている。
- [ ] PC名、二つ名、年齢、性別、PL名を空欄のまま編集できる。
- [ ] `設定`は初期状態で閉じ、操作で改行を保持するプレーンテキスト入力を表示・非表示できる。
- [ ] G4の全form valueは`null`非許容であり、文字列のdefault valueは`""`、信用の4入力のdefault valueは`0`である。
- [ ] 取得信用、融通した信用、融通された信用は`0`以上の整数だけを受け付け、小銭最大値修正は負数を許可する整数として編集できる。
- [ ] 信用の4入力が空欄になる操作では`0`へ戻り、使用可能信用と小銭最大値をその値で表示する。
- [ ] G4時点の消費信用は`0`と表示し、アイテム由来の集計や信用超過エラーを先行実装していない。
- [ ] Node testとPlaywrightで、信用の`0`以上制約、空欄操作後の`0`、小銭最大値修正の負数、信用の派生表示、設定の改行・開閉、label / keyboard操作を確認している。
- [ ] design targetとVRT baselineの扱いを記録し、PRレビュー直前にG4で確定したtargetだけをVisual Reviewする。
- [ ] 不要な依存を追加せず、`npm run check` と `npm run build` が通る。

## チェックポイント

- [ ] 既存ルートとG0〜G3のlayout・section frame操作を壊していない。
- [ ] GitHub Pagesのサブパス公開に影響しない。
- [ ] `react-hook-form`を唯一の編集stateとして維持し、値を`null`で初期化または更新していない。
- [ ] 文字列の空欄を`null`へ置換せず、信用の4入力だけは空欄になる操作を`0`へ正規化している。
- [ ] `docs/requirements/character-sheet.md`、`docs/design/character-sheet/notes.md`、`docs/TODO.md`と矛盾していない。
- [ ] 初期スコープ外の画像、dialog、保存、JSON、アイテム、作成ウィザードを混ぜていない。
...
- 文字列入力は空欄を許可して`null`ではなく`""`で初期化し、信用の4入力は`null`も空欄も保持せず`number`として扱える契約になっているか。取得・融通した・融通された信用の`0`以上制約と、小銭最大値修正の負数許可を確認したい。
