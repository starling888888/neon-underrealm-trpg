...
- スキル表示行と候補ダイアログの候補行は、名称を含むメタ情報を1行目へ横並びにし、効果本文だけを2行目へ置く。名称領域は、流儀・生き様・共通スキルの全生成スキル名の最大文字列を折り返さない幅とする。G12ではmobileでもこの幅を縮めず、他領域の表示崩れは実装後のactual確認で判断する。
...
- スキルの重複、最大レベル、流儀レベル合計、`advanced`条件、共通スキル上限の構造化エラーはG16で扱う。ただし、G12の候補表示はプライマリ流儀レベル6未満で`advanced`を候補に含めない。
