...
- [ ] `.raw/release-notes.xlsx` から `data/generated/release-notes.json` を既存変換scriptで更新している
- [ ] 更新済みJSONがリリースノート仕様のschema、日付降順・sourceOrder降順、ID規則に従う
- [ ] トップページの最新5件表示と `/release-notes/` の全件表示が同じ生成JSONを参照する既存仕様を維持している
- [ ] 現行リリースノート件数・文言に関するdesign notesを必要な範囲で整合させている
- [ ] `/` と `/release-notes/` の対象VRTで、`home` default / home-viewport / release-notes default のdesktop・tablet・mobileを確認している
- [ ] 各対象route・state・viewportで、リリースノート表示領域のtest-owned原寸locator screenshotを開き、文言の折返し、欠け、横overflow、既存レイアウトとの関係を確認している
- [ ] ユーザー承認済みの対象canonical VRT baselineだけを更新し、更新後に同じtargetの比較を再実行している
- [ ] `npm run check` が通る
- [ ] `npm run build` が通る
- [ ] `npm run test` が通る
- [ ] `npm run test:e2e` が通る
- [ ] `npm run build:public` と `npm run build:search-index` が公開手順の順序で通る
- [ ] 公開route、公開成果物、依存関係、外部service・server要件を `docs/out-of-scope.md` と照合し、未承認の初期スコープ外機能がないと記録している
- [ ] GitHub Releaseとversion tagを作成していない
...
- [ ] `docs/design/home/` と `docs/design/release-notes/` の既存design意図と矛盾していない
- [ ] `dist/` の公開成果物にprivate routeが残らず、検索indexが生成されている
