...
このissueはremote snapshot draftを元に作成したが、ローカルrepo上で `08-seo-component` branch、`docs/issue/done/phase-1/08-seo-component.md`、`public/neon-underrealm-ogp.png`、実装対象ファイル、検証コマンドを確認済みである。
...
- local validation: `.tmp/08-review.md` はPR #10のremote PR review draftであり、ローカル未検証項目を含むため鵜呑みにはしない。ローカル確認では、`docs/issue/done/phase-1/08-seo-component.md` にremote draft由来の記述が残っている一方、完了条件とチェックポイントは完了済みであることを確認した。`src/lib/utils/paths.ts` の `toAbsoluteUrl()` は `withBase()` を内部で呼んでおり、base付与済みパスを渡すと二重付与になり得るため、呼び出し契約の明文化は現在issueの範囲に含まれる有効指摘と判断する。

### 対応方針

- `docs/issue/done/phase-1/08-seo-component.md` のremote draft由来セクションを、実装後の検証済み状態に合わせて整理する。
