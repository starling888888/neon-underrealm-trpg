...
- 親issueの「初回公開時の構造」で、区切り線とサポートの間に置くべき「キャラクターシート」が抜けている。

### 判定

- source: `.tmp/chatgpt-review.md`（browser-draft）
- classification: valid
- local validation: 親issueの「区分見出し」はGMガイドの後に区切り線、キャラクターシート、サポートの順を契約している。一方「初回公開時の構造」は区切り線の直後がサポートになっており、現在のG1実装・子issueの契約と矛盾する。

### 対応方針

- 親issueの「初回公開時の構造」を、区切り線、キャラクターシート、サポートの順に訂正する。実装・メニュー・VRT baselineは変更しない。

### 対応完了チェックリスト

- [x] 親issueの「初回公開時の構造」にキャラクターシートを追加する。
