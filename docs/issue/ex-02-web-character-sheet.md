...
- `.tmp/character-sheet-requirements.md`: 初回実装の機能仕様
...
1. 機能要件の正本化
   - 既存サイトコンテンツ、既存要件、生成済みJSONの構造、`.tmp/character-sheet-requirements.md`を照合する。
   ...
   - `.tmp/character-sheet-requirements.md`に記載された機能は、同文書「2.3 初回実装のスコープ外」を除き、すべて初期scopeとして採用する。自動算出、マスタ選択、JSON入出力、端末内保存、画像、CCFOLIA出力を除外しない。
...
実装ゲートの詳細な分割と順序は、要件正本化・design・アーキテクチャ決定後にこのissueへ追加する。現時点で採用する実装スコープは、`.tmp/character-sheet-requirements.md`の「2.3 初回実装のスコープ外」を除く全機能である。
...
- [ ] `docs/requirements/character-sheet.md`が既存コンテンツ・データ・初回実装仕様を参照して正本化され、`docs/requirements.md`から参照できる。
- [ ] 要件・`src/pages/`配下のゲーム仕様・生成JSON・初期スコープ文書の競合と不明点が、ユーザーとの対話によって解消または明示的に初期スコープ外へ分類されている。
- [x] `.tmp/character-sheet-requirements.md`の「2.3 初回実装のスコープ外」を除く全機能を、初期scopeとして採用する。
...
- [ ] `docs/requirements/character-sheet.md`を追加する際、既存の要件索引と重複させずに参照関係を更新している。
...
- [ ] `src/pages/`配下のゲーム仕様と生成JSONを正本として、初回仕様の縁の項目、リアクションで選べる能力値、能力値配分、武器の所持・装備状態を整合させている。
- [ ] フルスクラッチにおけるプライマリ流儀・生き様の初期1レベル無料と、消費経験点の算出式を整合させている。
- [ ] `docs/requirements/overview.md`の「自動計算つきキャラクター作成フォームは初期スコープ外」と、`docs/plan.md`の`ex-02-web-character-sheet`の関係を正本で明確化している。
