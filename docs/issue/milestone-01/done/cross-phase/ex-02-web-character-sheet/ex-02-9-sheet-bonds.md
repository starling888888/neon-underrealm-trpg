...
- ユーザーの最新指示により、各縁のクリア操作は、角丸の横長`クリア` text buttonにする。visible textとaccessible nameは、この操作が行削除ではなく対象・関係・覚悟を初期状態へ戻すクリアであることを伝える。
...
- Gate plan: `docs/issue/milestone-01/done/cross-phase/ex-02-web-character-sheet/plan.md`の`G9`
...
- 各行は短い対象input、長い関係input、覚悟checkbox、角丸の横長`クリア` text buttonを持つ。覚悟済みでは対象と関係を編集不可にし、checkboxを解除すると再編集できる。クリアは行を削除せず、対象、関係、覚悟を初期値へ戻す。text buttonは、削除buttonと形状・visible textで誤認されないようにする。
...
- [x] 各行に対象、関係、覚悟checkbox、行削除と誤認しない角丸の横長`クリア` text buttonがあり、クリアで対象・関係・覚悟だけを初期値へ戻せる。
...
- クリアtext buttonが行削除と誤認されず、対象・関係・覚悟だけを初期化する操作として支援技術を含めて理解できるか。
...
- ユーザーの最新指示に合わせて、`docs/requirements/character-sheet.md`と`docs/design/character-sheet/notes.md`の覚悟効果の名称、順序、mobile配置、クリア操作、式表示を更新済みである。
