...
- Gate plan: `docs/issue/milestone-01/done/cross-phase/ex-02-web-character-sheet/plan.md` の `G7`
...
- プライマリ流儀または生き様が未選択の間も、格は両レベルの合計として初期値`2`を、成長可能点は格から算出して表示する。消費経験点と残経験点は、選択済みの流儀だけを集計して初期値`0`と`50`を表示する。プライマリ流儀が選択済みなら基礎能力値、体力増加値、精神力増加値、共通スキルボーナスを表示し、生き様が選択済みなら能力値ポイント候補、体力係数、精神力係数を表示する。常時能力値、一時能力値は両マスタが選択済みの場合だけ算出し、それ以外は`-`とする。生き様が未選択の能力値ポイント候補は`-`、5つの能力値ポイント入力はすべて`0`とし、未選択自体と能力値ポイントの一致はエラーにしない。生き様を選択しても能力値ポイントを自動配分・自動補正せず、入力を保持したまま生成JSONとの不一致を局所エラーで示す。選択後は通常の派生・検証へ切り替える。
- 未選択状態の表示契約は、両方未選択時に格`2`・成長可能点`0`・消費経験点`0`・残経験点`50`とし、常時能力値・一時能力値は`-`とする。プライマリ流儀だけ選択した場合は同流儀由来の基礎能力値、体力・精神力増加値、共通スキルボーナスを追加表示する。生き様だけ選択した場合は同生き様由来の能力値ポイント候補、体力・精神力係数を追加表示する。いずれの片選択状態でも、成長可能点は格から算出し、常時能力値・一時能力値は`-`とする。
...
- その他流儀の可変行は、各行で`その他流儀1`、`その他流儀2`、`Lv`を繰り返さない。その他流儀とLvを列ヘッダーとして示す。
- その他流儀の削除は、入力欄より背の高い強調buttonではなく、gray fillに白い`×`を置く小さな丸buttonにする。primary colorとdanger borderは使わない。
- 新規の数値入力からブラウザ標準のincrement / decrement controlを消さない。数値は3桁までを想定するため、キャラクターシート全体で表示を統一する。

### 判定

- source: human
- classification: valid
- local validation: 現行`BuildSection`はその他流儀の各行を`SelectField`と`BuildNumberInput`で構成するため、各行で選択labelとLv labelを表示している。`BuildSection.module.css`の`.otherRow`は入力欄の`1.5rem`より高い`1.75rem`の`.removeButton`を置き、danger色のborderと文字を用いる。同CSSは`.numberInput`へ`appearance: textfield`とWebKit spinner非表示を指定する一方、`ProfileSection.module.css`の取得経験点・信用のnumber inputにはその指定がなく、browser標準controlの有無が不統一である。`CharacterSheetFormPresenter.module.css`はキャラクターシート全体のform scopeを持つ。G7ではその他流儀が初期0行であり、削除時の対応スキル確認は後続Gateの範囲であるため、現時点の削除操作は行内の補助操作として扱える。`docs/design/character-sheet/notes.md`の可変行方針とも矛盾しない。

### 対応方針

- その他流儀が1行以上ある場合だけ、selectとレベル入力の上に`その他流儀`と`Lv`の列ヘッダーを表示する。行ごとの可視labelはなくし、操作対象を区別するaria-labelだけは行番号付きで維持する。
- 削除buttonは入力欄の高さを超えない小円形とし、grayの背景、whiteの`×`、枠線なしで中央揃えにする。buttonのaccessible nameは維持する。
- `CharacterSheetFormPresenter`のform scopeへnumber input共通styleを置き、G7のspinner抑制を外す。ブラウザ標準controlを保ったまま3桁想定の入力幅と右揃えを維持する。
- Component testで列ヘッダー、行ごとの可視label非表示、削除操作のcallbackを確認する。新しいVRT testcaseやcanonical baselineは追加・更新しない。

### 対応完了チェックリスト

- [x] その他流儀とLvを列ヘッダー化し、行ごとの可視labelを除く
- [x] 削除buttonをgray fillの小さな丸buttonへ変更し、accessible nameとcallbackを維持する
- [x] number inputのbrowser標準increment / decrement controlをキャラクターシート全体で統一する
- [x] Component testを更新する。新しいVRT testcaseは追加しない
- [x] 既存`@character-sheet` targetのVisual Reviewを実行し、canonical baselineを更新しない
- [x] `npm run check` が通る
- [x] `npm run build` が通る
...
- source: human
- classification: valid
- local validation: 現行`ProfileSection`は消費経験点・残経験点・格のすべてへ`FormulaTooltip`を付与している。`.tmp/design/character-sheet/tablet.png`はtablet（820px）で流儀・生き様入力を左、能力値を右に横並びとし、`BuildSection.module.css`の`width < 80rem`は現在これを縦積みにしている。draftの共通スキルボーナスはLv 2、Lv 5、Lv 9の獲得内容を個別cardとして示し、現在の`共通スキルLv / 上限`行はない。`CharacterSheetSectionFrame.module.css`の`overflow: clip`により、内側の絶対配置Tooltipがframe外へ出られない。`docs/issue/milestone-01/done/cross-phase/ex-02-web-character-sheet/plan.md`のG14は共通スキルを扱うGateであり、共通スキルLvと上限の基本情報側への最終配置をそこで接続できる。
...
- `calculateBuild`を、プライマリ流儀由来、生き様由来、両方由来の値に分けて返す。プライマリ流儀だけで決まる基礎能力値・体力／精神力増加・共通スキルボーナス、生き様だけで決まる能力値ポイント候補・体力／精神力係数、その他流儀を含めて独立に合算できる消費・残経験点は、片方が未選択でも表示する。格と成長可能点は選択状態にかかわらずプライマリ流儀レベルと生き様レベルから表示し、初期値はそれぞれ2と0とする。常時能力値、一時能力値だけを`-`にする。能力値ポイントの一致検証と成長検証も、それぞれに必要な選択値がある場合だけ実施し、入力値は保持する。
...
- [x] 片方だけ未選択のときも独立した派生値・経験点・格（初期値2）・成長可能点（初期値0）を表示し、常時能力値・一時能力値だけを未算出表示にする
- [x] `@character-sheet` targetのVisual Reviewを実行する
- [x] `npm run check` が通る
- [x] `npm run build` が通る
