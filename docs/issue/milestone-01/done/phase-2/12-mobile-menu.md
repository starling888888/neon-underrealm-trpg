...
`docs/issue/milestone-01/plan.md` のPhase 2では、`11-site-menu` の後続タスクとして `12-mobile-menu` が定義されている。

このissue内のplan ID / issue path参照は、実装当時の作業記録として保持する。現在このissueは完了済み履歴として `docs/issue/milestone-01/done/phase-2/12-mobile-menu.md` に退避済みである。
...
- 必要に応じて `docs/issue/milestone-01/done/phase-2/12-mobile-menu.md`
...
`docs/TODO.md` の「生成JSONとデータ取得層ができた後、サイドメニューに流儀リストと生き様リストを表示する」は、実装当時は旧plan ID `28-sample-generated-data` / `29-data-access-layer` 後に扱う前提として記録していた。現在は `29-0-ryugi-index-data` / `31-0-ikizama-index-data` 後に、必要なら `29-2-ryugi-index-page` / `31-2-ikizama-index-page` または別のナビゲーション補完タスクで扱う。このissueでは既存メニュー項目のスマホ表示・開閉導線のみを扱い、メニュー内容そのものをデータ駆動で拡張しない。
...
- issue file `docs/issue/milestone-01/done/phase-2/12-mobile-menu.md` をローカルで確認済み
- `docs/issue/milestone-01/plan.md` の `12-mobile-menu` 未完了タスクと一致
...
- `src/scripts/mobile-menu.ts` のfocus trap対象に、`tabindex="-1"` のscrim buttonが含まれる可能性がある。
- `BaseLayout.astro` のscrimは `button` かつ `tabindex="-1"` で、通常のTab移動対象から外す意図になっている。
- 一方で `focusableSelector` が `button:not([disabled])` を含むため、scrimは `tabindex="-1"` でも `focusableElements()` の候補に入る。
- その結果、focus trap上の `lastElement` が実際のTab順に存在しないscrimになり、最後のメニュー項目からTabした際にフォーカスがdrawer外へ抜ける可能性がある。

### 判定

- source: local-agent
- classification: valid
- local validation: `.tmp/12-review.md` の指摘をローカル実装と照合した。
- local validation: `src/layouts/BaseLayout.astro` でscrimに `data-mobile-menu-close`、`tabindex="-1"`、`button` が設定されていることを確認した。
- local validation: `src/scripts/mobile-menu.ts` の `focusableSelector` に `button:not([disabled])` が含まれ、filterでは `tabindex="-1"` を除外していないことを確認した。
- local validation: この指摘は `12-mobile-menu` の完了条件であるfocus trapに直接関係し、current issueで対応すべき内容である。

### 対応方針

- `focusableElements()` から `tabindex="-1"` の要素を明示的に除外する。
- 可能ならfocus trap対象をdrawer全体ではなく `.mobile-menu-panel` 内に限定し、scrimをfocus trapの操作対象から外す。
- 修正後、drawerを開いた状態でTab / Shift+Tabがdrawer内の操作可能要素で循環し、scrimがfocus trap対象に含まれないことを確認する。

### 対応完了チェックリスト

- [x] `tabindex="-1"` の要素がfocus trap対象から除外されている
- [x] focus trap対象がdrawer panel内の操作可能要素に限定されている、またはscrimが対象外であることを確認した
- [x] drawer表示中、Tab / Shift+Tab のフォーカス移動がdrawer内に収まる
- [x] `npm run check` が通る
- [x] `npm run build` が通る
