...
`docs/issue/milestone-01/plan.md` の `15-current-menu-highlight` は、Phase 2 の「現在ページハイライトを実装する」タスクである。
...
- [x] 現在表示中のページに対応するサイトメニューリンクが、PC左サイトメニューで視覚的に識別できる
- [x] 現在表示中のページに対応するサイトメニューリンクが、スマホdrawer内サイトメニューで視覚的に識別できる
- [x] exact matchするリンクには `aria-current="page"` が設定されている
- [x] exact matchしない詳細ページでは、存在しないリンクに `aria-current="page"` を付けず、最も近い親カテゴリだけをancestor状態として強調できる
- [x] 現在ページを含む親カテゴリは、初期表示時に展開または強調される
- [x] 既存の手動開閉操作は維持され、現在ページ親カテゴリであってもユーザー操作による開閉が破綻しない
- [x] パス比較はGitHub Pagesのサブパス公開に依存して壊れない
- [x] パス比較は末尾スラッシュ、query、hashの影響を受けない
- [x] ハイライトは色だけに依存せず、背景・文字色・font weight等の複数要素で識別できる
- [x] hover / focus 表示と現在ページハイライトが混同されない
- [x] `docs/TODO.md` の「生成JSONとデータ取得層ができた後、サイドメニューに流儀リストと生き様リストを表示する」は、このissueでは未対応のまま扱う
- [x] UI系タスクとして、参照するdesign targetとdesign画像の扱いが記録されている
- [x] design画像作成が必要な場合は、`design-image-generation` initial draft mode の実行を前提条件として記録している
- [x] `npm run build` が通る
- [x] `npm run check` が通る
...
- [x] 既存ルートが壊れていない
- [x] PC左サイトメニューとスマホdrawer内サイトメニューで同じ現在ページ判定を使っている
- [x] `SiteMenu.astro` / `SiteMenuItem.astro` の責務が過剰に膨らんでいない
- [x] `siteMenuItems` の手書きメニュー定義を不必要に複雑化していない
- [x] `withBase()` 済みhrefと現在パスを直接比較して、サブパス公開時に壊れる実装になっていない
- [x] `aria-current="page"` を親カテゴリやancestorに誤設定していない
- [x] 親カテゴリの強調が、現在ページそのもののハイライトより強くなりすぎていない
- [x] hover / focus / active / ancestor のCSSが互いに衝突していない
- [x] disclosureの `aria-expanded` と `hidden` の初期状態が、現在ページ親カテゴリの展開状態と矛盾していない
- [x] PageTocの現在位置ハイライトを実装していない
- [x] パンくずリスト、前後ナビゲーション、検索UIを追加していない
- [x] 不要な依存関係を追加していない
...
`15-current-menu-highlight` 実装中に、目視確認用ダミーMDXとして `/data`、`/data/items`、`/data/items/weapons` を追加した。該当ページの本実装時に削除または置き換える作業は `docs/TODO.md` で追跡する。
...
- issue: `docs/issue/milestone-01/done/phase-2/15-current-menu-highlight.md`
...
- `docs/issue/milestone-01/plan.md` の `15-current-menu-highlight` とbranch / issue名は一致している。
