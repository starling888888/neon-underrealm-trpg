...
- `docs/issue/milestone-01/plan.md` の `50-responsive-pass`。本 issue はそのうち Header と desktop のレールスクロール挙動だけを扱う子タスクとする。
...
- CSS と JavaScript 間のブレークポイント定数一元化は、初期スコープ外として `docs/out-of-scope.md` に記録した。
- CSS custom property を media query へ直接使えない制約があり、一元化には `@custom-media` のビルド時変換または定義生成の仕組みが必要になる。
- 値の重複による保守性リスクは認識するが、現時点では導入コストに対する効果が見合わないため意図的に許容する。将来は単一入力から CSS と JavaScript の定義を生成または検証する方針を候補とする。
