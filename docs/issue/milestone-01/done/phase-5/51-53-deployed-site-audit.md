...
`docs/issue/milestone-01/plan.md` の `51-performance-pass`、`52-github-pages-base-check`、
...
- `docs/issue/milestone-01/plan.md` の `51-performance-pass`、`52-github-pages-base-check`、`53-content-smoke-test`
...
- `docs/issue/milestone-01/plan.md` のチェックボックス更新と関連TODOの完了・退避

## 完了条件

- [x] Gate G1〜G3の公開サイト調査結果が、条件ごとの合格・失敗・未確認・適用外と証跡を含めて記録されている。
- [x] 調査時点の公開artifactと現在のmainの差異、または差異を確認できなかった理由が記録されている。直近成功deployは `a9dd6e1`（2026-07-31 06:28:30 UTC完了）で、公開homeの `Last-Modified` は同時刻帯だった。現在のmain `c9d39b9` はその直後の1 commitで、差分は `docs/issue` のrenameのみのため公開出力は同一である。
- [x] 性能、subpath、主要導線・検索・個別アンカーの各失敗が、公開artifact不一致、実装、workflow、外部配信・キャッシュ、調査環境に分類されている。
- [x] 修正が必要な項目は、修正対象・根拠・影響範囲・回帰確認・design判断の要否を含む修正計画として記録され、ユーザーへ報告されている。今回のG1〜G3調査では修正対象なしのため、修正計画は作成しない。
- [x] ユーザー承認前には、調査目的のソース、テスト、workflow、design正本、VRT baselineを変更していない。
- [x] 修正を実施した場合だけ、対象のローカル検証とmain deploy後の公開URL再確認が完了している。修正対象なしのため適用外。
- [x] 関連TODOを扱った場合は、対応結果またはこのissueで保留する理由が記録されている。
- [x] `npm run check`、`npm run build` は、修正を実施した場合だけ実行結果を記録している。ソース修正なしのため適用外。
...
- [x] hero画像の表示領域、非hero画像のlazy loading、キャラクターシート以外で初期表示を妨げる不要なclient JavaScript、カード一覧の過剰なclient描画、外部解析スクリプトを確認している。通常35 routeのbrowser inventoryで確認済み。
- [x] 画像・スクリプトなどの外部assetが、混在コンテンツ・CORS・404などにより主要導線を壊していない。36 routeのbrowser resource確認で外部origin要求は0件だった。
