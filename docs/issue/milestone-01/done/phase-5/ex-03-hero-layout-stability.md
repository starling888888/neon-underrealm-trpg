...
`docs/issue/milestone-01/plan.md` の `ex-03-hero-layout-stability` は、ヒーロー画像の表示遅延によるコンテンツ位置ずれを防ぐtaskである。
...
- `docs/issue/milestone-01/plan.md` の `ex-03-hero-layout-stability`
- `docs/design/home/`
- `docs/design/header-footer/`
- `docs/issue/milestone-01/done/phase-3/18-1-common-image-block-component.md`
...
- `docs/issue/milestone-01/plan.md` の完了チェック更新
...
- local validation: `tests/visual/hero-layout-stability.spec.ts`は`width` / `height`と表示可否を確認するのみで、遅延request中の後続要素位置、`naturalWidth`、response成功をassertしていない。`docs/issue/milestone-01/plan.md`の`53-content-smoke-test`には検索確認があるため、検索Visual Testの安定化を同taskへ紐付けた。
