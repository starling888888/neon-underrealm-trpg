...
`docs/issue/milestone-01/plan.md` の Phase 0 にある初期化タスクとして、サイトコードを配置できる最小構成が必要である。
...
- [x] Astroプロジェクトの最小構成が作成されている
- [x] `package.json` が作成され、開発・ビルドに必要なscriptsが定義されている
- [x] `tsconfig.json` が作成され、TypeScript strict 前提の設定になっている
- [x] 最小ページが静的サイトとして表示できる
- [x] `npm run build` が通る
...
- [x] 既存ドキュメントを破壊していない
- [x] GitHub Pagesのサブパス公開に関する本格対応は次タスク以降に残している
- [x] 不要な依存関係を追加していない
