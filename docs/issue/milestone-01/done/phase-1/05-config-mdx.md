...
このサイトでは、ルール本文や世界観本文をMarkdown / MDXで継続管理する方針である。`docs/issue/milestone-01/plan.md` のPhase 1では、Astro基盤としてMDX integrationの追加、`.mdx` ページの表示確認、MDX内Component埋め込み方針の確認が予定されている。
...
- [x] Astro MDX integrationが追加され、`astro.config.mjs` に設定されている
- [x] `.mdx` ページが1つ以上追加され、Astroの静的ページとして表示できる
- [x] MDX内でAstro Componentを埋め込む最小例が確認できる
- [x] MDX内Component埋め込み方針がドキュメント化されている
- [x] 追加依存関係の理由、代替案、初期スコープに必要な理由を作業報告に記載できる
- [x] `npm run build` が通る
- [x] `npm run check` が通る
...
- [x] 既存ルート `/` が壊れていない
- [x] GitHub Pagesのサブパス公開に影響する設定変更をしていない
- [x] 不要な依存関係を追加していない
