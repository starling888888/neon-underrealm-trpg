...
* トップページ `/` や404ページ `/404` など、ページ内目次が不要な画面では表示しない
...
トップページや404ページのような入口・状態表示ページでは、ページ内目次を表示しない方が自然である。
...
* 画面によってはページ内目次を表示しない
* トップページ `/` と404ページ `/404` ではページ内目次を表示しない
...
* 既存の `id` がある場合は原則として尊重する
* ただし、TOC対象見出しの `id` がASCII-only制約に違反する場合は検出する
...
見出しに既に `id` がある場合は、原則としてそのIDを尊重する。

ただし、TOC対象見出しの既存 `id` が日本語などの非ASCII文字を含む場合は検出対象とする。
...
* Astro integrationの `astro:build:done` hookで実行する
* `scripts/postprocess-page-toc.mjs` などの独立scriptを作り、`npm run build` に組み込む

どちらを採用するかは実装時に判断する。

### HTMLパーサ

build後のHTMLを安全に編集するため、HTMLパーサの導入を検討する。
...
* `src/lib/site/postprocess-page-toc.ts`
* `scripts/postprocess-page-toc.mjs`
* `astro.config.mjs`
* `package.json`
...
トップページ `/`、404ページ `/404`、その他目次が不要な画面ではページ内目次を表示しない。
...
* トップページ `/` と404ページ `/404` を明示的にTOC対象外にする方針でよいか
