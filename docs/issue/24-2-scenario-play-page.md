...
- キャラクターロストの確認と縁の清算を、シナリオ終了後の処理として配置する。
...
- [ ] ユーザー提供hero画像 `public/images/scenario-play/hero.webp` をH1直後に表示し、指定alt、`loading="eager"`、captionなし、ページ側の追加overlayなしを守っている
- [ ] `/rules/scenario-play.mdx` を作成し、既存のMDX Layoutと共通Componentを利用している
- [ ] `.raw/contents/scenario-play.md` の本文・HTMLコメントと矛盾しない
- [ ] シーン進行と、開始前、オープニング、ミドル、情報収集、中間戦闘、休息、クライマックス、エンディング、GMシーンの役割を掲載している
- [ ] ミドルシーンが情報収集、中間戦闘、休息を必要回数組み合わせて進むことを掲載している
- [ ] 情報収集が各PCにつき1シーン1回の判定であり、達成値によって得られる情報が変わりうることを掲載している
- [ ] 休息が戦闘後に少なくとも1回行われ、登場PCの体力・精神力を最大まで回復し、消耗品は回復しないことを掲載している
- [ ] GMシーンでPLが得た情報とPCが知る情報を区別することを掲載している
- [ ] 通常のシーンでは、登場したキャラクター1人と毎シーン必ず縁を結ぶことを掲載している
- [ ] シーン中の縁が最大4つであり、入れ替え、エンディングシーンと自PCだけのシーンの例外を掲載している
- [ ] 情報収集の`example` Calloutに、仁義への技能変更と判定数4から14の例を掲載している
- [ ] 小銭を、初期作成時の所持信用10からアイテム・装備に使った信用を引いてシナリオ開始時に確定することを掲載している
- [ ] 小銭が非戦闘技能判定後にだけ使え、消費分だけ達成値を上げ、判定数・効果値は変えず、戦闘判定には使えないことを掲載している
- [ ] 小銭がシナリオ中に増えず、使った分だけ減り、所持信用やアイテム状態から再計算しないことを掲載している
- [ ] 戦闘準備の装備組み換えが、現在装備・所持アイテムに使った信用と未使用小銭の範囲に制限されている
- [ ] 戦闘準備で、使用済みまたは効果適用済みのシナリオ回数制アイテムを変更できないことを掲載している
- [ ] シナリオ終了後のキャラクターロスト確認で、死亡、すべての縁を覚悟にして足を洗うこと、死亡したPCの魂魄を得て外道堕ちを選ぶことの3条件を掲載している
- [ ] シナリオ終了後の縁の清算で、縁を1つ残し、次のシナリオで再び縁を結べることを掲載している
- [ ] 経験点・信用の獲得と覚悟の累積を掲載していない
- [ ] Visual Reviewの結果を、ユーザー確認なしにdesign正本へコピーしていない
- [ ] `npm run check` が通る
- [ ] `npm run build` が通る

## チェックポイント

- [ ] 提供hero画像の右下に焼き込まれたゲームロゴをページ側で重ねて表示せず、隠蔽もしていない
- [ ] `/rules`、`/rules/battle`、`/advancement` への内部リンクがGitHub Pagesのサブパス公開で壊れない
- [ ] PageTocとMobilePageTocへ不要なCallout titleを見出しとして含めていない
- [ ] desktop / mobileで本文、hero、Calloutに横overflowや可読性の低下がない
- [ ] 既存ルート、SiteMenu、PageToc、MobilePageToc、共通Calloutを壊していない
- [ ] 不要な依存関係を追加していない
- [ ] シナリオ本文、ハンドアウト本文、キャンペーン本文、GMガイドを混入させていない
- [ ] 経験点・信用獲得をこのページへ戻していない
- [ ] 関連する `docs/TODO.md` 項目と矛盾していない
- [ ] `docs/design/rules/`、`docs/design/callout/`、`docs/design/site-layout/`、`docs/design/page-toc/`、`docs/design/mobile-page-toc/` の共通design制約と矛盾していない
