...
* `docs/design/site-menu/`
  ...
  * このタスクの直接参照target
  * remote snapshot上では未確認
  * 実装前にこのIssue内で作成またはローカル確認が必要
...
* `docs/design/site-menu/` は未作成
* `npm run check` / `npm run build` は未実行

`docs/design/site-menu/` が未作成のため、実装開始前に `design-image-generation` initial draft modeで `docs/design/site-menu/notes.md` と `docs/design/site-menu/design-desktop.png` を作成し、人間レビューを受ける必要がある。
