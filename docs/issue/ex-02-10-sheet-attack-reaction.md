...
- `判定`sectionの列ヘッダーは左から`技能`、`対応能力`、`判定数`とする。`判定数`の横には、`常時／一時`をmute colorで常時表示する。
...
- 攻撃判定は初期1行、最低1行とする。技能候補は`喧嘩`、`暗殺`、`発砲`、`格闘`、`干渉`とし、各行に削除buttonを置く。最後に残る1行の削除buttonは操作不可とし、行数を0にしない。追加操作で攻撃行を増やせるようにする。
- 攻撃行とリアクション行の`対応能力`は、筋力、敏捷、感覚、肉体、精神の5能力値から選択・変更できる。技能を選んだときの既定対応は、`喧嘩／筋力`、`暗殺／敏捷`、`発砲／感覚`、`格闘／肉体`、`干渉／精神`とする。攻撃の初期行は`喧嘩／筋力`とする。追加行も`喧嘩／筋力`で開始する。
...
- [ ] `判定`sectionは`技能`、`対応能力`、`判定数`をヘッダーに持ち、`判定数`の横にmute colorの`常時／一時`を表示できる。
- [ ] `判定数`headerから、指定どおりの計算式と手動修正の説明を含むtooltipを確認できる。
- [ ] 攻撃とリアクションの各行が、`常時能力値 / 一時能力値 + 修正入力欄 = 常時判定数 / 一時判定数`を示し、修正の変更で両判定数を更新できる。
- [ ] 攻撃は初期1行・最低1行で追加・削除でき、最後の1行は削除できない。リアクション4行は常時表示され、追加・削除できない。
- [ ] 技能・リアクション種別ごとの指定既定対応を表示し、対応能力を5能力値から変更できる。
- [ ] 判定数の算出、能力値変更・修正変更、攻撃の追加・削除・最低1行、リアクション固定4行、tooltip、desktop / tablet / mobileの表示を、pure logic、schema / hook、Component、browser behavior testの適切な層で確認している。
...
- [ ] `docs/design/character-sheet/notes.md`と、ユーザー指定を優先する判定sectionの配置・列・式表示契約を混同していない。
...
- `技能`、`対応能力`、`判定数`とmute colorの`常時／一時`が、各数値が常時・一時で対になることを過不足なく示せているか。
