...
- [ ] Markdown style ruleが、見出しATX形式、`-`、2 spaces indentの3点に限定されている
- [ ] Google Markdown Style GuideをMarkdown style判断の正本として扱う記述が削除または無効化されている
- [ ] Fetch MCPは残っているが、Markdown style情報取得には使わない方針になっている
- [ ] Markdown formatterがdev dependencyとして導入されている
- [ ] 追加packageの追加理由、代替案、初期スコープに必要な理由が記録されている
- [ ] `format:md` が追加されている
- [ ] `check:md` または同等のMarkdown format未適用検出手段が追加されている
- [ ] `npm run format` からMarkdown formatterが実行される
- [ ] `npm run check` または専用checkでMarkdown format未適用を検出できる
- [ ] `.md` ファイルを作成・編集したらMarkdown formatterをかける運用がエージェント向けruleに記録されている
- [ ] Git管理対象の既存 `.md` ファイルを一度formatしている
- [ ] `.mdx` をformat対象に含めた場合、MDX構文が壊れていないことを確認している
- [ ] `.raw/`、`.tmp/`、生成物、Visual Review成果物、build成果物がformat対象外になっている
- [ ] `npm run check` が通る
- [ ] `npm run build` が必要な変更範囲の場合は通る
- [ ] `docs/agent-failure-log.md` で3回以上積み重なっている同種失敗の有無を作業報告で報告している
- [ ] commit / pushを実行していない

## チェックポイント

- [ ] `AGENTS.md` のissue-first停止条件を弱めていない
- [ ] `.agents/skills/issue-first-development/SKILL.md` の停止地点を弱めていない
- [ ] `.agents/rules/mcp.md` でFetch MCP自体を消していない
- [ ] Markdown style ruleがGoogle Markdown Style Guideより広い規約になっていない
- [ ] formatter導入によってAstro / TypeScript / Biomeの既存format/checkが壊れていない
- [ ] `package-lock.json` が追加packageと整合している
- [ ] 既存ルートが壊れていない
