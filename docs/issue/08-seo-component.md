...
* [ ] 共通OGP画像として `public/neon-underrealm-ogp-1200x630-v2.png` を参照する方針になっている
...
* `public/neon-underrealm-ogp-1200x630-v2.png`

上記のうち、`src/lib/site/seo.ts`、`src/lib/utils/paths.ts`、既存ページへの導入は必要な場合のみ変更する。

共通OGP画像アセットはユーザー提供の `public/neon-underrealm-ogp-1200x630-v2.png` を使用する。実装時に別名や別画像へ差し替える場合は、issueの方針変更としてユーザー確認を行う。
...
そのため、通常のUIデザイン画像は原則不要と判断する。共通OGP画像そのものはユーザー提供の `public/neon-underrealm-ogp-1200x630-v2.png` を使用し、このissueでは画像生成を行わない。
