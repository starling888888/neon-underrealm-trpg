...
- [ ] `.github/workflows/deploy.yml`が`npm run check`を実行する。
- [ ] `.github/workflows/deploy.yml`が`npm run build:public`の後に`npm run build:search-index`を実行する。
- [ ] Pagefind生成物を含む`dist/`がGitHub Pages artifactとしてuploadされる。
- [ ] `docs/deployment.md`と`docs/requirements/architecture.md`が、検索indexの生成・artifactへの組込み・deployを正しく説明する。
- [ ] ローカルで`npm run build:public`、`npm run build:search-index`の順に成功し、`dist/pagefind/pagefind.js`を確認する。
- [ ] GitHub Pagesの公開環境で検索UIがPagefind indexを参照し、検索結果を表示できる。
- [ ] GitHub Pagesサブパス`/neon-underrealm-trpg/`配下で検索indexのpathが壊れない。
- [ ] `npm run check`が通る。

## チェックポイント

- [ ] workflowのartifact upload対象が検索index生成後の`dist/`のままである。
- [ ] CI/CDはExcel、`.raw/`、外部検索サービス、Secretsに依存しない。
- [ ] `src/scripts/search-modal.ts`の`import.meta.env.BASE_URL`による`pagefind/`参照と、生成物の配置が整合する。
- [ ] 既存ルートとGitHub Pagesのサブパス公開に影響しない。
- [ ] 不要な依存関係を追加していない。
- [ ] 初期スコープ外の機能を実装していない。
- [ ] 関連する`docs/TODO.md`項目と矛盾していない。
- [ ] ユーザーの未コミット変更（`public/images/data/`配下の画像を含む）を破壊・stage・commitしない。
...
- ローカル検証では生成物の存在とbase path対応を確認する。GitHub Pages上の実検索はdeploy後の人間確認を必要とするため、確認できるまで該当完了条件は未チェックにする。
