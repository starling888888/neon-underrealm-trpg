...
* 実装では、スマホ固定ページタイトルバー右側の控えめな目次triggerと、固定ページタイトルバー直下に開く軽いoverlay表示に従う
...
* スマホ固定ページタイトルバー右側の控えめな目次trigger
* 固定ページタイトルバー直下に表示する軽いoverlay UI
* 既存のTOC生成結果の再利用
* TOC項目リンクによるページ内ジャンプ
...
* スマホ幅では、サイトHeaderとは別に現在ページタイトルを表示する固定ページタイトルバーを置く
* 固定ページタイトルバーの右側に、ボタン感を弱めた目次triggerを置く
* triggerは大きな枠付きbuttonではなく、短いラベル、控えめなchevron、必要最小限のaccentで操作可能性を示す
* 開いた状態では、固定ページタイトルバー直下に軽いoverlayとしてページ内見出しリンク一覧を表示する
...
* [ ] スマホ幅で固定ページタイトルバーに現在ページタイトルが表示されている
* [ ] 固定ページタイトルバー右側の控えめな目次triggerからMobilePageTocを開閉できる
* [ ] MobilePageTocは固定ページタイトルバー直下の軽いoverlayとして表示される
...
* [ ] 参照するdesign targetとdesign画像の扱いが実装報告に記録されている
* [ ] `npm run build` が成功する
...
* Headerのmenu buttonとMobilePageTocのtriggerが別機能だと分かる
...
* スマホ固定ページタイトルバーを追加し、右側に控えめな目次triggerを置く方針でよいか
...
* MobilePageTocは固定ページタイトルバー直下の軽いoverlayとして表示する方針でよいか
...
採用designは、スマホ固定ページタイトルバー右側の控えめな目次triggerと、固定ページタイトルバー直下に開く軽いoverlay表示とする。
