...
- リストのmarkerがアクセント色で強く、本文中でやや主張しすぎている
- デスクトップ表示でテーブル右側に背景色の合わない余白が出ており、表の幅・寄せ方が不自然に見える
- `mdx-test.mdx` のテーブル確認サンプルがMarkdown記法ではなくHTMLの `<table>` になっている
- `mdx-test.mdx` に `<main className="prose">` を直接書いているが、後続Layout対応時に移動する前提を明確にしたい

### 対応方針

- リストmarkerはアクセント色を弱めるか、本文向けの落ち着いた色へ調整する
- テーブルはデスクトップで不要な右側余白が出ないようにし、横幅いっぱい固定ではなく内容幅と横スクロールの扱いを見直す
- `mdx-test.mdx` の表サンプルは、まずMarkdown table記法でMDX build/checkが通るか確認し、通る場合はMarkdown tableへ変更する
- `<main className="prose">` は今回のLayout未導入段階の一時対応として残す場合、後続Layoutタスクで移動するTODOを明記する
- 修正後にPlaywrightスクリーンショットを再取得し、目視確認する。機械的な画像diffは行わない

### 対応完了チェックリスト

- [x] リストmarkerの色を本文中で強すぎない見た目に調整した
- [x] テーブル右側の不自然な余白を解消または軽減した
- [x] `mdx-test.mdx` の表サンプルをMarkdown記法にできるか確認し、可能なら変更した
- [x] `main.prose` の扱いについて、後続Layout移行前提のTODOまたは記録を追加した
- [x] desktop screenshot を再取得し、目視確認した
- [x] mobile screenshot を再取得し、目視確認した
- [x] `npm run check` が通る
- [x] `npm run build` が通る
...
- 現在のスクリーンショットでは、本文密度が少ない状態でH1下線、H2/H3左罫線、引用左罫線、リンク、リストmarkerなどにアクセントカラーが重なり、画面全体のアクセントが強く見える
- TRPGのルールブックとして長文を読む画面では、青緑アクセントが常時表示されすぎると目に厳しい可能性がある
- 見出しや引用の形は維持しつつ、本文装飾の差し色は濃いグレー寄りにして、アクセントカラーはリンクやサイドバーの現在ページハイライトなど操作・現在地表示に限定したい
- リストmarkerもアクセントカラーではなくグレー寄りにしたい

### 対応方針

- `tokens.css` で操作アクセントと本文装飾用の色を分離する
- `--color-accent` はリンク、フォーカス、現在ページハイライトなどの操作・状態表示向けとして残す
- 見出し罫線、引用罫線、リストmarkerなどのprose装飾には、濃いグレーまたは低彩度の本文装飾用tokenを使う
- 見出しや引用の形、余白、基本構造は維持し、色の強さを落とす範囲に留める
- `docs/design/global-styles/style-tile.png` と `style-tile-mobile.png` を再生成し、design正本と実装方針の齟齬が残らないようにする
- 修正後にPlaywrightスクリーンショットを再取得し、目視確認する。機械的な画像diffは行わない

### 対応完了チェックリスト

- [x] 操作アクセントと本文装飾用の色tokenを分離した
- [x] 見出し罫線、引用罫線、リストmarkerの色をグレー寄りに調整した
- [x] リンクやフォーカスなど操作対象にはアクセントカラーを残した
- [x] 見出しや引用の形、余白、基本構造を維持した
- [x] `docs/design/global-styles/style-tile.png` と `style-tile-mobile.png` を再生成した
- [x] desktop screenshot を再取得し、目視確認した
- [x] mobile screenshot を再取得し、目視確認した
- [x] `npm run check` が通る
- [x] `npm run build` が通る
