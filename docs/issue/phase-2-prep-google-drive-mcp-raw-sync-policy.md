...
* [ ] このissueが、目的、背景、対象範囲、初期スコープ外、完了条件、チェックポイント、想定変更ファイル、レビュー観点、備考を含む形で確定している
* [ ] `docs/issue/phase-2-prep-google-drive-mcp-raw-sync-policy.md` が作成されている
* [ ] `docs/plan.md` へタスク追加していない
* [ ] `AGENTS.md` にGoogle Drive正本と `<repo-root>/.raw/` 同期方針が追加されている
* [ ] `.agents/skills/drive-to-raw-sync/SKILL.md` が追加されている
* [ ] 必要に応じて `.agents/skills/README.md` に `drive-to-raw-sync` の使用条件が追加されている
* [ ] 必要に応じて `.agents/rules/data-management.md` にGoogle Drive正本と `.raw` 同期方針が追加されている
* [ ] 必要に応じて `.agents/rules/mcp.md` にDrive MCP利用時の制約が追加されている
* [ ] 必要に応じて `.agents/rules/file-structure.md` に `<repo-root>/.raw/` の固定構造が追加されている
* [ ] `.gitignore` に `raw-google-drive.url` が追加されている
* [ ] `README.md` に、開発時はリポジトリルート直下へ `raw-google-drive.url` を追加することが追記されている
* [ ] Google Drive同期対象フォルダのURLを `<repo-root>/raw-google-drive.url` で管理する方針が明記されている
* [ ] `raw-google-drive.url` をGit管理しない方針が明記されている
* [ ] `.raw/` が常に `<repo-root>/.raw/` を指すことが明記されている
* [ ] OSルート直下の `/.raw/` ではないことが明記されている
* [ ] カレントディレクトリ基準の `./.raw/` ではないことが明記されている
* [ ] 同期前に `git rev-parse --show-toplevel` でrepo rootを確認することが明記されている
* [ ] `<repo-root>/.raw/` 配下の構造が `release-notes.xlsx` / `data/*.xlsx` / `contents/*.md` に固定されている
* [ ] Google Drive側も同期対象フォルダ直下で同じ構造を取ることが明記されている
* [ ] `<repo-root>/.raw/sheets/` を使わないことが明記されている
* [ ] Drive側とローカル側の相対pathを対応させることが明記されている
* [ ] Google Drive MCPを第一候補として使う方針が明記されている
* [ ] Google DocsはMarkdown `.md` として取得する方針が明記されている
* [ ] Google SheetsはExcel `.xlsx` として取得する方針が明記されている
* [ ] Drive MCPから取得したcontentをCodex / 生成AIエージェントがローカルfilesystemへ保存する責務分離が明記されている
* [ ] `<repo-root>/.raw/` をGit管理しない既存方針が維持されている
* [ ] Drive上の正本を書き換えないことが明記されている
* [ ] Drive書き込み系toolを使用しないことが明記されている
* [ ] Drive MCPが使えない場合は停止し、代替同期スクリプトを勝手に作らないことが明記されている
* [ ] CI/CDにGoogle Drive同期を含めないことが明記されている
* [ ] 認証情報をrepoに置かないことが明記されている
* [ ] `<repo-root>/.raw/` は作業入力であり、公開サイトの直接SSoTではないことが明記されている
* [ ] `src/pages/**/*.mdx` / `.astro` が最終的なページ本文・UI構造のSSoTであることと矛盾していない
* [ ] `data/generated/**/*.json` が生成物であり、手編集しない方針と矛盾していない
* [ ] 必要に応じて `docs/content-writing-guide.md` が更新されている
* [ ] `npm run build` が通る
* [ ] 必要に応じて `npm run check` が通る

## チェックポイント

* [ ] 既存の `.raw/` Git管理除外方針を壊していない
* [ ] `raw-google-drive.url` をGit管理対象にしていない
* [ ] `raw-google-drive.url` の場所が `<repo-root>/raw-google-drive.url` に固定されている
* [ ] `<repo-root>/.raw/` 以外への書き出し余地がない
* [ ] `.raw/contents/*.md` を作業入力にする既存方針と矛盾していない
* [ ] Google Driveを公開サイトの直接SSoTとして扱っていない
* [ ] Codex / 生成AIエージェントがページ作成前に迷わず同期できる粒度になっている
* [ ] 同期手順が曖昧すぎない
* [ ] 同期手順が実装詳細に踏み込みすぎていない
* [ ] 独自Google Drive APIスクリプト実装へ膨らんでいない
* [ ] `rclone` 等の特定CLIを必須前提にしていない
* [ ] Drive MCPがない場合の停止条件が明確である
* [ ] Drive書き込み操作を禁止している
* [ ] Drive上の正本を変更・削除・移動しない
* [ ] ローカル `.raw/` からDriveへ書き戻さない
* [ ] 双方向同期を導入していない
* [ ] 削除同期を導入していない
* [ ] 同名ファイル衝突を黙って解決しない
* [ ] Drive側の想定外pathを勝手に推測配置しない
* [ ] `<repo-root>/.raw/sheets/` を作成しない
* [ ] 変換失敗時に既存ファイルを破壊しない
* [ ] 空ファイルで上書きしない
* [ ] 取得失敗した古い `.raw` 入力を最新として扱わない
* [ ] 認証情報、token、credentialをrepoに置かない
* [ ] CI/CDでDrive MCPやGoogle Driveを参照しない
* [ ] CMS、DB、管理画面、APIサーバーへ膨らんでいない
* [ ] Phase 2前準備として妥当な粒度に収まっている
* [ ] 既存ルートが壊れていない
