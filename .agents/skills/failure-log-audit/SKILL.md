...
description: Use this skill when the user asks to audit docs/agent-failure-log/active.md, identify failure categories with three or more occurrences, summarize risks, propose where permanent countermeasures should be added, discuss a plan with the user, and only implement approved countermeasures after explicit confirmation.
---

# Failure Log Audit Skill

Audit `docs/agent-failure-log/active.md` and propose permanent countermeasures.
...
Move entries from `docs/agent-failure-log/active.md` to `docs/agent-failure-log/done.md` only after approved countermeasures are implemented and the user confirms the entries are handled.
...
When the user explicitly judges an entry to need no permanent countermeasure, move it from `docs/agent-failure-log/active.md` to `docs/agent-failure-log/no-action.md`.
